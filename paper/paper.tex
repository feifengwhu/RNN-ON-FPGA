...
Therefore, the network~\cite{Chang15} is able to perform around \num{34.3d3} forward propagations per second.
Although the network is large, its performance is well below this work, which achieves \num{487d3} forward propagations per second, for that network size.
...
\end{equation}
This parameters represents the number of rows that share the same multiplier. The
